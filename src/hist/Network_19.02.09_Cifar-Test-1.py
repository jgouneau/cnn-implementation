...
class Network:
    ...
    def training_batch(self, file):
        ...
        for k in range(10000):
            ...
            for d in range(3):
                for i in range(32):
                    img[d, i, :] = batch[b'data'][k, 1024*d + 32*i : 1024*d + 32*(i+1)]
...
class ConvSigmoid:
    ...
    def  __init__(self, rg, parameters, prev_size):
        ...
        self.bias = np.random.randn(D2) * 0.01 / sqrt(D1*H1*W1)
...
class Pooling:
    ...
    def back(self, prev_layer):
        ...
        for d in range(D):
            for h in range(H):
                for w in range(W):
                        M = prev_layer.output[d, h*S:h*S+F, w*S:w*S+F] - self.output[d, h, w]
                        M[M == 0] = 1
                        M[M != 1] = 0
...
class FullyMax:
    ...
    def front(self, inp):
        ...
        for d in range(D):
            for h in range(H):
                for w in range(W):
                    self.output[d, h, w] = max(0, np.sum(self.weigths[d, h, w] * inp) + self.bias[d, h, w]) + 0.01*min(0, np.sum(self.weigths[d, h, w] * inp) + self.bias[d, h, w])
...
class ConvMax:
    ...
    def  __init__(self, rg, parameters, prev_size):
        ...
        self.weigths = np.random.randn(D2, D1, F, F) * 0.01 / sqrt(D1*H1*W1)
        self.delta = np.zeros((D2, D1, F, F))
        self.bias = np.ones((D2)) * 0.01 / sqrt(D1*H1*W1)
        self.delta_bias = np.zeros((D2))
        self.term = np.zeros(self.size)
        self.output = np.zeros(self.size)
        self.speed, self.moment, self.white = rate
    ...
    def front(self, inp):
        ...
        for d in range(D):
            for h in range(H):
                for w in range(W):
                    self.output[d, h, w] = max(0, np.sum(self.weigths[d] * padded_inp[:, h*S:h*S+F, w*S:w*S+F]) + self.bias[d]) + 0.01*min(0, np.sum(self.weigths[d] * padded_inp[:, h*S:h*S+F, w*S:w*S+F]) + self.bias[d])
...
def Test_Deep_Conv():
    lay_list = [(Input, (3, 32, 32)),
                (ConvMax, (5, 5, 1, -1, (0.05, 0.7, 0.0005))),
                (Pooling, (2, 2)),
                (ConvMax, (5, 5, 1, -1, (0.05, 0.7, 0.0005))),
                (Pooling, (2, 2)),
                (ConvMax, (5, 5, 1, -1, (0.05, 0.7, 0.0005))),
                (FullyMax, (4, 1, 1, (0.05, 0.7, 0.0005)))]
    cat_list = [0, 1, 2]
...
def Test_Simple_Conv():
    ...
    lay_list = [(Input, volume),
                (ConvMax, (5, 8, 2, 0, (0.01, 0.3, 0.0001))),
                (ConvMax, (5, 5, 1, -1, (0.01, 0.3, 0.0001))),
                (ConvMax, (5, 5, 1, -1, (0.005, 0.1, 0.0001))),
                (ConvMax, (2, 5, 1, 0, (0.001, 0.03, 0.0001)))]
    cat_list = ["True", "False"]
    
    net = Network("Simple_Conv", lay_list, cat_list)
    net.define()
...
def The_Grand_Test():
	lay_list = [(Input, (3, 32, 32)),
                (ConvMax, (16, 5, 1, 2, (0.01, 0.9, 0.0005))),
                (Pooling, (2, 2)),
                (ConvMax, (20, 5, 1, 2, (0.01, 0.9, 0.0005))),
                (Pooling, (2, 2)),
                (ConvMax, (20, 5, 1, 2, (0.01, 0.9, 0.0005))),
                (Pooling, (2, 2)),
