...
class Pooling:
    ...
    def  __init__(self, rg, parameters, prev_size):
        Pooling.ident += 1
        self.id = "Pooling n°" + str(Pooling.ident)
        self.rank = rg
        D1, H1, W1 = prev_size
        F, S = parameters
        ...
        for d in range(D):
            for h in range(H):
                for w in range(W):
                    for i in range(H1):
                        for j in range(W1):
                            # on envoie le terme d'erreur dans le neurone précedent s'il sagit du neurone responsable
                            if self.output[d, h, w] == prev_layer.output[d, i, j] :
                                prev_layer.term[d, i, j] += self.term[d, h, w]
...
lay_list = [(Input, volume),
            (Convolutional, (5, 8, 2, 0, (0.1, 0.9, 0.0005))),
            (Convolutional, (5, 5, 1, -1, (0.1, 0.9, 0.0005))),
            (Convolutional, (2, 5, 1, 0, (0.1, 0.9, 0.0005)))]
cat_list = ["True", "False"]
...
lay_list = [(Input, volume),
            (Convolutional, (5, 5, 1, -1, (0.1, 0.9, 0.0005))),
            (Convolutional, (2, 5, 1, -1, (0.1, 0.9, 0.0005))),
            (FullyConnected, (1, 1, 2, (0.5, 0.9, 0.0005)))]
cat_list = ["True", "False"]
